...
for _, name in ipairs(peripheral.getNames()) do
    ...
    elseif ptype == "monitor" then table.insert(allMons, name) end
end
table.sort(allGpus)
table.sort(allMons)
...
local function loadConfig()
    if fs.exists(CONFIG_FILE) then
        ...
        if type(data) == "table" then
            ...
            if data.deviceMap then for k,v in pairs(data.deviceMap) do deviceMap[k]=v end end
        end
    end
end
...
local function checkRegistration()
    ...
    for _, name in ipairs(allGpus) do
        ...
        pcall(function()
            ...
            local w, h = g.getSize()
            local msg = "CHECKING..."
            local tx = math_max(1, math_floor(w/2) - math_floor(#msg * 6))
            local ty = math_floor(h/2) - 8
            pcall(g.drawText, tx, ty, msg, 0xFFFF00, 0x050A05, 2)
            g.sync()
        end)
    end
...
if not isRegistered then showNotRegisteredAndHalt() return end
        ...
        for i=1,poolCount do
            ...
            if t.col and t.col~=C.BLACK then
                ...
                if (px-cx)^2+(py-cy)^2<=r*r then
                    if t.isBeacon then
                        gpuDrawTinyTriangle(g,px,py,t.col)
                    else
...
local function drawAsdicSector(entry)
    ...
    g.line(cx, cy, lx, ly, 0x00CC44); g.line(cx, cy, rx, ry, 0x00CC44)
    local step = 2
...
local function rdrGpuUI()
    ...
    local entry = {gpu = radarGpu, cx = cx, cy = cy, r = r, w = w, h = h, dotSize = ds, lastSweepDeg = -9999}
    local frames = 0
    ...
    while true do
        if currentScreenTab == 4 then
            if radarGpu then
                radarGpu.fill(C.BG)
                pcall(radarGpu.drawText, cx - 6, cy - 4, peripheral.getName(radarGpu), C.WHITE, C.BG, 2)
                radarGpu.sync()
            end
            sleep(0.3)
        else
            frames = frames + 1
            local isActive = radarEnabled and (currentRadarRange > 0) and isServoConnected
            ...
            if localPos and isActive then
                    ...
                    if data.source == 8888 and data.lastPainted and not data.isBeacon then
                        ...
                        if age < TARGET_FADE_DURATION then
                            ...
                            if col and col ~= C.BLACK then
                                ...
                                if not t then t = {}; targetPool[poolCount] = t end
                ...
                for _, data in pairs(targets) do
                    if data.source == 8888 and data.isBeacon and data.lastSeen and (now - data.lastSeen < 5.0) and data.realPos then
                        ...
                        local distRatio = (currentRadarRange > 0) and math_min(dist/currentRadarRange, 1.0) or 1.0
            ...
            if forceRedraw or angleDiff > 0.5 or frames <= 2 or hasTargets or hasRwr then
                ...
                pcall(gpuRefreshRadar, entry, isActive, poolCount, targetPool)
            end
            sleep(0.05)
        end
    end
...
-- ==========================================
-- 声纳 GPU 主循环（独立，无IFF/RWR）
-- ==========================================
local function asdicGpuUI()
    if not asdicGpu then return end
    ...
    local targetPool = {}
    local poolCount = 0
    local lastAsdicScanAngle = -9999

    while true do
        if currentScreenTab == 4 then
            if asdicGpu then
                asdicGpu.fill(0x050A05)
                pcall(asdicGpu.drawText, cx - 6, cy - 4, peripheral.getName(asdicGpu), 0xFFFFFF, 0x050A05, 2)
                asdicGpu.sync()
            end
            sleep(0.3)
        else
                    ...
                    if data.source == 8889 and data.lastPaintedAsdic and not data.isBeacon then
                        ...
                        if age < TARGET_FADE_DURATION then
                            ...
                            if col and col ~= 0x000000 then
                                local screenRad = math_rad(data.paintedYaw)  -- 已经为相对方位
            ...
            local angleDiff = math_abs(getAngleDiff(asdicScanAngle, lastAsdicScanAngle))
            if forceRedraw or angleDiff > 0.5 or frames <= 2 or poolCount > 0 then
                lastAsdicScanAngle = asdicScanAngle
                pcall(refreshAsdic, entry, isActive, poolCount, targetPool)
            end
            sleep(0.05)
        end
    end
...
local lastRadarHud = {sText="", rText="", lText="", dText="", iffMode=""}
local function drawRadarHUD()
    if not radarHud then return end
    local m = radarHud; m.setTextScale(1); m.setBackgroundColor(colors.black)
    local dw, dh = m.getSize()
    ...
    if sText==lastRadarHud.sText and rText==lastRadarHud.rText and lText==lastRadarHud.lText and dText==lastRadarHud.dText and iffMode==lastRadarHud.iffMode then return end
    lastRadarHud = {sText=sText, rText=rText, lText=lText, dText=dText, iffMode=iffMode}
    m.clear()
    ...
    if dw>=5 and dh>=5 then pcall(hudDrawIffCorners, m, dw, dh, 1) end
end

-- 声纳 HUD
local lastAsdicHud = {line1="", line3=""}
local function drawAsdicHUD()
    if not asdicHud then return end
    local m = asdicHud; m.setTextScale(1); m.setBackgroundColor(colors.black)
    local dw, dh = m.getSize()
    ...
    if line1==lastAsdicHud.line1 and line3==lastAsdicHud.line3 then return end
    lastAsdicHud = {line1=line1, line3=line3}
    m.clear()
    local function cprint(y,text,col)
        m.setTextColor(col)
        m.setCursorPos(math_max(1, math_floor((dw-#text)/2)+1), y)
...
local function termUI()
    while true do
        term.setBackgroundColor(colors.black)
        term.clear()
        local tabs = {"RADAR", "ASDIC", "STATUS", "DEVICES"}
        local tabStr = ""
        ...
        elseif currentScreenTab == 2 then
            ...
            drawInputBox(5, "Heading Off  :", headingOffset, menuIndex==1, isEditing)
        ...
        elseif currentScreenTab == 3 then
            ...
            if #allGpus == 0 then term.setCursorPos(4,row); term.setTextColor(colors.red); term.write("None"); row=row+1
            else
                for idx, name in ipairs(allGpus) do
                    local role = deviceMap[name] or "none"
                    local label = ROLE_LABELS[role] or role
                    local isSel = (menuIndex == 100 + idx)
                    ...
                    if isSel then term.setCursorPos(4+#name+2,row); term.write("<") end
                    row=row+1
                end
            end
            row=row+1
            ...
            if #allMons == 0 then term.setCursorPos(4,row); term.setTextColor(colors.red); term.write("None"); row=row+1
            else
            ...
            row=row+1
            term.setCursorPos(2,row); term.setTextColor(colors.gray)
            term.write("UP/DOWN select, ENTER cycle role")
        end
        sleep(0.2)
    end
end
...
local function inputLoop()
    ...
    local function cycleDeviceRole(devName, isGpu)
        ...
        local currentIdx = 1
        for i, r in ipairs(roles) do if r == currentRole then currentIdx = i; break end end
    ...
    while true do
        ...
        if event == "key" then
            if p1 == keys.tab then
                currentScreenTab = (currentScreenTab % 4) + 1
                menuIndex = 1
            elseif isEditing then
                if currentScreenTab == 1 or currentScreenTab == 2 then
                    if p1 == keys.enter or p1 == keys.numPadEnter then
                        if currentScreenTab == 1 then applyRadarSave() else applyAsdicSave() end
                ...
                elseif currentScreenTab == 2 then
                    ...
                    elseif p1 == keys.enter or p1 == keys.numPadEnter then
                        isEditing = true
                        if menuIndex == 1 then inputStr = tostring(headingOffset)
                        elseif menuIndex == 2 then inputStr = tostring(ASDIC_SCAN_ANGULAR_SPEED)
                        elseif menuIndex == 3 then inputStr = broadcastOwnPos and "yes" or "no"
                        end
                    end
        ...
        elseif event == "char" and isEditing then
            local tab = currentScreenTab
            if tab == 1 or tab == 2 then
                local isText = false
                if tab == 1 and menuIndex == 1 then isText = true
                elseif tab == 2 and menuIndex == 3 then isText = true end
                if isText then
        ...
        elseif event == "mouse_click" then
            ...
            if currentScreenTab == 1 then
                ...
                elseif touchY == 11 then ti = 4 end
                if ti then
                    if isEditing and menuIndex ~= ti then
                        if currentScreenTab == 1 then applyRadarSave() else applyAsdicSave() end
                    end
                    menuIndex = ti; isEditing = true
                    if currentScreenTab == 1 then
                        if ti == 1 then inputStr = radarEnabled and "yes" or "no"
                        elseif ti == 2 then inputStr = tostring(yawOffset)
                        elseif ti == 3 then inputStr = tostring(motorOffset)
                        elseif ti == 4 then inputStr = tostring(aimPrecision) end
                    end
                else
                    if isEditing then
                        if currentScreenTab == 1 then applyRadarSave() else applyAsdicSave() end
            ...
            elseif currentScreenTab == 2 then
                ...
                if touchY == 5 then ti = 1
                elseif touchY == 7 then ti = 2
                elseif touchY == 9 then ti = 3 end
                if ti then
                    if isEditing and menuIndex ~= ti then applyAsdicSave() end
                    menuIndex = ti; isEditing = true
                    if ti == 1 then inputStr = tostring(headingOffset)
                    elseif ti == 2 then inputStr = tostring(ASDIC_SCAN_ANGULAR_SPEED)
                    elseif ti == 3 then inputStr = broadcastOwnPos and "yes" or "no" end
                else
            ...
            if radarGpu and peripheral.getName(radarGpu) == touchedName and localPos and currentRadarRange > 0 and radarEnabled and isServoConnected then
                ...
                for id, data in pairs(targets) do
                    if data.source == 8888 and data.lastPainted and not data.isBeacon and (now - data.lastPainted < TARGET_FADE_DURATION) then
                        ...
                        local py = cy - math_floor(r * distRatio * math_cos(yawRad) + 0.5)
                        local dSq = (mx-px)^2 + (my-py)^2
                        if dSq <= minSqDist then
                            if not bestId or dSq < bestDist then
                                bestId = id; bestDist = dSq
                            end
                        end
                    end
                end
                ...
                for id, data in pairs(targets) do
                    if data.source == 8888 and data.isBeacon and data.lastSeen and (now - data.lastSeen < 5.0) and data.realPos then
                        ...
                        local dist = calcRangingDist(localPos, data.realPos) or 0
                        local ratio = (currentRadarRange>0) and math_min(dist/currentRadarRange,1.0) or 1.0
                        local yawRad = math_rad(tYaw + yawOffset)
                        local px = cx + math_floor(r * ratio * math_sin(yawRad) + 0.5)
                        local py = cy - math_floor(r * ratio * math_cos(yawRad) + 0.5)
                        local dSq = (mx-px)^2 + (my-py)^2
                ...
                if bestId then
                    if iffMode == "friendly" then
                        ...
                        selectedTargetDistStr = string.format("%dm", math_floor(bestDist and (bestDist*currentRadarRange) + 0.5 or 0))
                    end
                end
            ...
            elseif asdicGpu and peripheral.getName(asdicGpu) == touchedName and localPos and isAsdicActive then
                ...
                for _, data in pairs(targets) do
                    if data.source == 8889 and data.lastPaintedAsdic and not data.isBeacon then
                        ...
                        local py = cy - math_floor(r * distRatio * math_cos(screenRad) + 0.5)
                        local d2 = (mx - px)^2 + (my - py)^2
                        if d2 < bestDist then bestDist = d2; bestId = data.id end
                    end
                end
                if bestId then selectedTargetId = bestId
                else selectedTargetId = nil end
            end
...
local function pingLoop()
    while true do
        if currentScreenTab == 4 then sleep(0.5) else
            if localPos then
