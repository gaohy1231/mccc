--[[ 雷达系统(船只端)   v2.0.0  改为  GHG 被动定向水听器 v3.0
     改动：目标线、动态距离、监听线、测速、HUD 换行
--]]
...
local TARGET_FADE_DURATION = 3.0         -- 改为 8.0 方便观察
...
-- 动态深度范围参数
local SEA_LEVEL_Y    = -4       -- 请根据服务器实际海平面修改（F3 查看）
...
local CONFIG_FILE = "radar_config.txt"   -- 文件名不改，以免丢失配置
...
local currentStressCapacity = 0      -- 不再使用
local currentRadarRange     = 0      -- 改为动态水听范围
local currentNorthYawDeg    = 0
local currentScreenTab      = 1
local menuIndex             = 1
local isEditing             = false
local inputStr              = ""
local cachedStressometer    = nil     -- 不再使用
local cachedServo           = peripheral.find("servo")
local targetPool            = {}
local targetPoolCount       = 0
local iffMode               = "enemy"
local rwrEvents             = {}      -- 保留，但不再添加
...
-- ==========================================
-- 算法函数（保留全部，包括 quat, rotateVectorFast 等）
-- ==========================================
...
local C = {
    BG          = 0x000000,     -- 背景黑色
    OUTER_RING  = 0xFFCC00,     -- 外圈绿（原雷达颜色保留）
    INNER_RING  = 0x997700, 
    GRID        = 0x332200,
    SWEEP       = 0x000000,     -- 不再使用
    YELLOW      = 0xFFFF00,
...
local function checkRegistration()
    ...
    print("Radar v2.0.0 - Registration Check")
...
local function gpuFillRect(g, x, y, w, h, color)   -- 保留，未使用
    for dy = 0, h-1 do g.line(x, y+dy, x+w-1, y+dy, color) end
end
local function gpuDrawTinyTriangle(g, cx, cy, color)   -- 保留，未使用
...
local function gpuDrawSweep(entry, angleDeg)   -- 改为监听线
    local g=entry.gpu; local cx=entry.cx; local cy=entry.cy; local r=entry.r
...
local function gpuDrawTargetLine(entry, yawRad, color, targetData)
    ...
    if targetData then
        ...
        if targetData.speed then
            speedStr = string.format("%.1fkm/h", targetData.speed * 3.6)
        end
...
local function gpuRefreshRadar(entry, isActive, poolCount, pool)
    ...
    if isServoConnected then
        gpuDrawSweep(entry, currentServoAngle)   -- 监听线
    end
...
-- ==========================================
-- RDR GPU 主循环（修改：构建 targetPool 时改为目标线数据）
-- ==========================================
local function rdrGpuUI()
    ...
    while true do
        if currentScreenTab==2 then
            ...
            frames=frames+1
            local isActive = (currentRadarRange>0)   -- 是否激活（有范围即可，不依赖舵机）
...
local function hudMonitorUI()
    ...
    while true do
        ...
        local isActive=(currentRadarRange>0)   -- 不再依赖舵机
        for _,info in ipairs(hudMonitorList) do
...
local function termUI()
    while true do
        ...
        if currentScreenTab==2 then
            ...
            term.write(string.format("Max Range  : %.0f m", currentRadarRange))
            ...
            term.write((#rdrGpuList>0)
                and ("SONAR GPU: "..(#rdrGpuList).." online")
                or  "SONAR GPU: NONE")
            ...
            if entry and localPos then
                ...
                for id,data in pairs(targets) do
                    if data.paintedYaw then
                        ...
                        local nearX = entry.cx + t*dx
                        local nearY = entry.cy + t*dy
                        local distSq = (mx - nearX)^2 + (my - nearY)^2
                        if distSq <= 16 then   -- 点击精度
...
-- ==========================================
-- 网络协议（删除 pingLoop，仅保留 listenLoop 并修改）
-- ==========================================
                    ...
                    for id,data in pairs(targets) do
                        if data.realPos then
                            ...
                            if id == selectedTargetId then
                                ...
                                if inSector and dist <= currentRadarRange then
                                    data.lastPainted = now
                                else
                                    -- 让目标自然淡出（不立即清除 lastPainted）
                                end
                            end
                        end
                    end
...
local function speedTimerLoop()
    while true do
        ...
        if event == "timer" and timerId == speedTimer then
            ...
            for id, data in pairs(targets) do
                if data.lastPos and data.lastTime then
                    ...
                    if dt > 0.5 then
                        ...
                        data.speed = math_sqrt(dx*dx + dy*dy + dz*dz) / dt   -- m/s
                        -- 径向速度
            ...
            speedTimer = os.startTimer(SPEED_INTERVAL)   -- 重新启动定时器
        end
    end
end
...
term.setCursorPos(1,1)
term.setTextColor(colors.green)
print("GHG Sonar v3.0 - Passive")
print(string.format("  Name      : %s", myLabel))
print(string.format("  Dynamic Range (y=%d: %d, y=%d: %d)", SEA_LEVEL_Y, RANGE_SURFACE, DEEP_Y, RANGE_DEEP))
print(string.format("  GPU       : %d", #rdrGpuList))
print(string.format("  HUD       : %d", #hudMonitorList))
sleep(1.0)
